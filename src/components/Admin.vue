...
export default {
  ...
  methods: {
    ...
    resetPassword(rowdata) {
      ...
      axios.post('http://127.0.0.1:8080/fsims/admin/reset', qs.stringify({type, account}),{
        headers: {
    ...
    deleteUser(rowdata) {
      const type = rowdata.type
      const account = rowdata.account
      axios.post('http://127.0.0.1:8080/fsims/admin/deleteuser', qs.stringify({type, account}),{
        headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                }
      }).then(
